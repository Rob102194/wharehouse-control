...
import { DispatchRestaurantForm } from "@/app/(dashboard)/operations/dispatch-restaurant/dispatch-restaurant-form";
...
type DispatchRestaurantPageProps = {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export default async function DispatchRestaurantPage({ searchParams }: DispatchRestaurantPageProps) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Despachar al restaurante</h2>
          <p className="text-slate-600">Registra una salida de inventario desde el almacen activo.</p>
        </div>
        <Link
      ...
      {selectedWarehouse ? (
        <DispatchRestaurantForm warehouse={selectedWarehouse} variants={variants} />
      ) : (
