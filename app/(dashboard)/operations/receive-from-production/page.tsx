...
import { ReceiveFromProductionForm } from "@/app/(dashboard)/operations/receive-from-production/receive-from-production-form";
...
type ReceiveFromProductionPageProps = {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export default async function ReceiveFromProductionPage({ searchParams }: ReceiveFromProductionPageProps) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Recibir desde elaboracion</h2>
          <p className="text-slate-600">Registra ingreso de elaborados como entrada al almacen activo.</p>
        </div>
        <Link
      ...
      {selectedWarehouse ? (
        <ReceiveFromProductionForm warehouse={selectedWarehouse} variants={variants} />
      ) : (
