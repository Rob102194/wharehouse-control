...
export function ReceivePurchaseForm({ warehouse, variants }: ReceivePurchaseFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <OperationLineItemsForm
        ...
        emptyMessage="Busca y selecciona productos para agregar a la compra."
      />

      <label className="block space-y-1 text-sm">
        <span className="font-medium text-slate-700">Nota de compra (opcional)</span>
        <input
          name="notes"
          placeholder="Proveedor, factura o referencia"
