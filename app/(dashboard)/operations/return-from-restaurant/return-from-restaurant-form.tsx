...
type ReturnFromRestaurantFormProps = {
  warehouse: Warehouse;
...
export function ReturnFromRestaurantForm({ warehouse, variants }: ReturnFromRestaurantFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <OperationLineItemsForm
        ...
        emptyMessage="Busca y selecciona productos devueltos."
      />

      <label className="block space-y-1 text-sm">
        <span className="font-medium text-slate-700">Nota de devolucion (opcional)</span>
        <input
          name="notes"
          placeholder="Motivo o referencia de devolucion"
      ...
      <OperationSubmitButton label="Confirmar devolucion" pendingLabel="Registrando devolucion..." />
    </form>
  );
}
