...
import { TransferOutForm } from "@/app/(dashboard)/operations/transfer-out/transfer-out-form";
...
type TransferOutPageProps = {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export default async function TransferOutPage({ searchParams }: TransferOutPageProps) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Transferir a otro almacen</h2>
          <p className="text-slate-600">Registra una transferencia con origen fijo en el almacen activo.</p>
        </div>
        <Link
      ...
      {selectedWarehouse ? (
        <TransferOutForm warehouse={selectedWarehouse} warehouses={warehouses} variants={variants} />
      ) : (
